...
suite("Filesystem Utilities", () => {
  const tmpDir = path.join(os.tmpdir(), `clama-test-${Math.random().toString(36).slice(2)}`);
  ...
  suite("createDirectoriesForFile", () => {
    ...
    test("should normalize paths", async () => {
      ...
      createdDirs.length.should.equal(process.platform === "win32" ? 2 : 1);
