...
import { COMMAND_REQ_APP_STRING, combineCommandSequences } from "../shared/combineCommandSequences";
...
import { type AssistantMessageContent, type ToolParamName, type ToolUseName, parseAssistantMessage } from "./assistant-message";
import { constructNewFileContent } from "./assistant-message/diff";
import { parseMentions } from "./mentions";
import { formatResponse } from "./prompts/responses";
import { SYSTEM_PROMPT, addUserInstructions } from "./prompts/system";
import { getNextTruncationRange, getTruncatedMessages } from "./sliding-window";
import { type ClineProvider, GlobalFileNames } from "./webview/ClineProvider";
...
export class Cline {
  ...
  private consecutiveAutoApprovedRequestsCount = 0;
  ...
  private async saveClineMessages() {
    try {
      ...
      console.error("Failed to save clama messages:", error);
    }
  }
  ...
  async ask(
    ...
    if (this.abort) {
      throw new Error("Clama instance aborted");
    }
    let askTs: number;
  ...
  private async initiateTaskLoop(userContent: UserContent, isNewTask: boolean): Promise<void> {
    ...
    while (!this.abort) {
      ...
      if (didEndLoop) {
        ...
        // this.say(
        // 	"tool",
        // 	"Clama responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
        // )
        nextUserContent = [
          {
            type: "text",
  ...
  async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    ...
    if (await fileExistsAtPath(clineRulesFilePath)) {
      try {
        ...
        if (ruleFileContent) {
          clineRulesFileInstructions = `# .clamarules\n\nThe following is provided by a root-level .clamarules file where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${ruleFileContent}`;
        }
      } catch {
        console.error(`Failed to read .clamarules file at ${clineRulesFilePath}`);
      }
    }
              ...
              if (block.partial) {
                ...
                if (this.shouldAutoApproveTool(block.name)) {
                  ...
                  showNotificationForApprovalIfAutoApprovalEnabled(
                    `Clama wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`,
                  );
                  this.removeLastPartialMessageIfExistsWithType("say", "tool");
            ...
            try {
              if (block.partial) {
                ...
                if (this.shouldAutoApproveTool(block.name)) {
                  ...
                  showNotificationForApprovalIfAutoApprovalEnabled(`Clama wants to read ${path.basename(absolutePath)}`);
                  this.removeLastPartialMessageIfExistsWithType("say", "tool");
                  const didApprove = await askApproval("tool", completeMessage);
                  if (!didApprove) {
                    await this.saveCheckpoint();
            ...
            try {
              if (block.partial) {
                ...
                if (this.shouldAutoApproveTool(block.name)) {
                  ...
                  showNotificationForApprovalIfAutoApprovalEnabled(
                    `Clama wants to view directory ${path.basename(absolutePath)}/`,
                  );
                  this.removeLastPartialMessageIfExistsWithType("say", "tool");
                  const didApprove = await askApproval("tool", completeMessage);
                  if (!didApprove) {
            ...
            try {
              if (block.partial) {
                ...
                if (this.shouldAutoApproveTool(block.name)) {
                  ...
                  showNotificationForApprovalIfAutoApprovalEnabled(
                    `Clama wants to view source code definitions in ${path.basename(absolutePath)}/`,
                  );
                  this.removeLastPartialMessageIfExistsWithType("say", "tool");
                  const didApprove = await askApproval("tool", completeMessage);
                  if (!didApprove) {
            ...
            try {
              if (block.partial) {
                ...
                if (this.shouldAutoApproveTool(block.name)) {
                  ...
                  showNotificationForApprovalIfAutoApprovalEnabled(
                    `Clama wants to search files in ${path.basename(absolutePath)}/`,
                  );
                  this.removeLastPartialMessageIfExistsWithType("say", "tool");
                  const didApprove = await askApproval("tool", completeMessage);
                  if (!didApprove) {
              ...
              if (block.partial) {
                ...
                if (action === "launch") {
                  ...
                  if (this.shouldAutoApproveTool(block.name)) {
                    ...
                    showNotificationForApprovalIfAutoApprovalEnabled(`Clama wants to use a browser and launch ${url}`);
                    this.removeLastPartialMessageIfExistsWithType("say", "browser_action_launch");
            ...
            try {
              if (block.partial) {
                ...
                if (!requiresApproval && this.shouldAutoApproveTool(block.name)) {
                  ...
                  showNotificationForApprovalIfAutoApprovalEnabled(`Clama wants to execute a command: ${command}`);
  ...
  async recursivelyMakeClineRequests(userContent: UserContent, includeFileDetails = false, isNewTask = false): Promise<boolean> {
    if (this.abort) {
      throw new Error("Clama instance aborted");
    }
    ...
    if (this.consecutiveMistakeCount >= 3) {
      if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
        showSystemNotification({
          subtitle: "Error",
          message: "Clama is having trouble. Would you like to continue the task?",
        });
      }
      const { response, text, images } = await this.ask(
        ...
        this.api.getModel().id.includes("claude")
          ...
          : "Clama uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.5 Sonnet for its advanced agentic coding capabilities.",
      );
      if (response === "messageResponse") {
        userContent.push(
          ...[
    ...
    if (this.autoApprovalSettings.enabled && this.consecutiveAutoApprovedRequestsCount >= this.autoApprovalSettings.maxRequests) {
      if (this.autoApprovalSettings.enableNotifications) {
        showSystemNotification({
          ...
          message: `Clama has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests.`,
        });
      }
      await this.ask(
        "auto_approval_max_req_reached",
        `Clama has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`,
      );
    ...
    // biome-ignore lint: TODO remove this when we have a better way to handle this
    ...
    try {
      ...
      const updateApiReqMsg = (cancelReason?: ClineApiReqCancelReason, streamingFailedMessage?: string) => {
        this.clineMessages[lastApiReqIndex].text = JSON.stringify({
          ...
          cost:
            totalCost ?? calculateApiCost(this.api.getModel().info, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens),
          cancelReason,
  ...
  async loadContext(userContent: UserContent, includeFileDetails = false) {
